...
import styles from './Dzidza.module.css';
import UserTime from "../userWorkTime/UserTime.jsx";
import { dzidzaSchedule } from "../../data";

const Dzidza = () => {
  const { activeSchedule, handleActiveSchedule, handleActiveChildAges, activeChildAges, handleNumberOfDays, numberOfDays, CalcCompensation, handleHourCompensation, handleMultiLingual } = useContext(JobContext);
    ...
    <div style={{ display: "flex", flexDirection:"column", gap:"25px" }}>
      ...
      <div className={styles.child_age} style={handleChildAge(childAgeStatus) ? {display:"none"} : {display: "flex"}}>
        ...
        <select name="childAges" id="childAges" className={styles.options} onChange={(e) => handleActiveChildAges(e.target.value)}>
          <option value="newbornBaby">ახალშობილი</option>
          <option value="ertwlamde">1 წლამდე</option>
          <option value="atsplius">1-10+ წლამდე</option>
